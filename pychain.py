...
@dataclass
class Record:
    # Datastructure for the class Record
    sender: str
    receiver: str
    amount: float
...
#   consists of an attribute named `record`, which is set equal to a `Record` that contains
#   the `sender`, `receiver`, and `amount` values. The updated `Block`should also include
